...
stock <- dd$stock_code
tac <- inla.group(log(dd$tac_change+1), 500, method = 'cut')
...
# INLA formula 
form <- Y ~ 0 + mu +
  f(year, model='rw1', hyper = list(theta = list(prior="pc.prec", param=c(0.5,0.01))), scale.model=TRUE, constr=TRUE) +
  f(tac, model='rw1', hyper = list(theta = list(prior="pc.prec", param=c(0.5,0.01))), scale.model=TRUE, constr=TRUE) + 
  f(stock, model='iid', hyper=list(theta=list(param=c(0.1,0.1))), constr=TRUE)
